...
export const Agenda: React.FC<AgendaProps> = ({ onClose }) => {
  const [clients, setClients] = useLocalStorage<Client[]>("tm_clients", []);
  ...
  const sendWhatsApp = (appt: Appointment) => {
    ...
    setAppointments((prev) =>
      prev.map((a) =>
        a.id === appt.id
          ? { ...a, notified: { ...a.notified, whatsapp: true } }
          : a,
      ),
    );
  };
  ...
  const sendEmail = (appt: Appointment) => {
    ...
    setAppointments((prev) =>
      prev.map((a) =>
        a.id === appt.id
          ? { ...a, notified: { ...a.notified, email: true } }
          : a,
      ),
    );
  };
      ...
      >
        ...
        <div className="p-6 border-b border-stone-100 flex items-center justify-between bg-stone-50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black tracking-tighter text-stone-900 uppercase">
                Agenda Interactiva
              </h2>
              <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">
                Gestión de Visitas y Notificaciones
              </p>
            </div>
          </div>
            ...
            <>
              ...
              <div className="w-80 border-r border-stone-100 p-6 bg-stone-50/50">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-black text-stone-900 uppercase text-sm tracking-widest">
                    Seleccionar Fecha
                  </h3>
                ...
                <div className="grid grid-cols-7 gap-1 mb-8">
                  {["D", "L", "M", "M", "J", "V", "S"].map((d) => (
                    <div
                      key={d}
                      className="text-[10px] font-black text-stone-400 text-center py-2"
                    >
                      {d}
                    </div>
                  ))}
      ...
      <AnimatePresence>
        {showAddModal && (
          <div className="fixed inset-0 z-[110] bg-stone-950/60 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-2xl font-black text-stone-900 mb-6 uppercase tracking-tighter">
                Programar Visita
              </h3>
              <form onSubmit={handleAddAppointment} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <select
                    name="clientId"
                    required
                    className="w-full p-3 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                  >
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    className="flex-1 py-4 bg-stone-100 text-stone-500 rounded-2xl font-black uppercase tracking-widest text-[10px]"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-4 bg-emerald-500 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-500/20"
                  >
                    Guardar
                  </button>
                </div>
